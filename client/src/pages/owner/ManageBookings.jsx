...
const ManageBookings = () => {
    ...
    const fetchOwnerBookings = async ()=>{
      try {
        ...
        console.log("Error at catch of fetchOwnerBookings in ManageBookings")
    ...
    const changeBookingStatus = async (bookingId, status)=>{
      try {
        ...
        if(data.success){
          ...
          console.log("Error at else changeBookingStatus of ManageBookings")
          toast.error(data.message)
        }
      } catch (error) {
        console.log("Error at catch of changeBookingStatus of ManageBookings")
        toast.error(error.message)
      }
    }
            ...
            {bookings.map((booking, index)=>(
              <tr key={index} className='border-t border-borderColor text-gray-500'>
                ...
                <td className='p-3 flex items-center gap-3'>
                  <img src={booking.device.image} alt="" className='h-12 w-12 aspect-square rounded-md object-cover'/>
                  <p className='font-medium max-md:hidden'>{booking.device.brand} {booking.device.model}</p>
                </td>
