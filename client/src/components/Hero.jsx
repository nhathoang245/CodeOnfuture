...
import {assets, addressList} from '../assets/assets'
...
const Hero = () => {
    ...
    const handleSearch = (e)=>{
        e.preventDefault()
        navigate('/devices?pickupLocation=' + pickupLocation + '&pickupDate=' + pickupDate + '&returnDate=' + returnDate)
    }

  return (
    <div className='h-screen flex flex-col items-center justify-center gap-14 bg-light text-center'>
        
        <h1 className='text-4xl md:text-5xl font-semibold'>Futuristic devices on Hands</h1>
            ...
            <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8'>
                <div className='flex flex-col items-start gap-2'>
                    <select required value={pickupLocation} onChange={(e)=>setPickupLocation(e.target.value)}>
                        ...
                        {addressList.map((address)=> <option key={address} value={address}>{address}</option>)}
                    </select>
        ...
        <img src={assets.main_device} alt="device" className='max-h-74'/>
    </div>
  )
}
